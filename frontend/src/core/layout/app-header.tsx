...
export function AppHeader() {
    ...
    const menuItems = [
        ...
        // Ouvre désormais une confirmation, cohérent avec le comportement déjà en place sur /settings.
        { label: 'Déconnexion', onClick: () => setIsLogoutConfirmOpen(true), variant: 'danger' as const },
    ]
    ...
    return (
        <>
                        ...
                        {navItems.map((item) => (
                            <Link
                                ...
                                className={clsx(
                                    ...
                                    location.pathname === item.path ? 'text-ivory font-medium' : 'text-muted hover:text-ivory',
                                )}
                            >
                                {item.label}
                                {location.pathname === item.path && <span className="absolute -bottom-1 left-0 right-0 h-0.5 bg-amber rounded-full" />}
                            </Link>
                        ))}
                    </nav>
                </div>
            ...
            <Modal isOpen={isLogoutConfirmOpen} onClose={() => setIsLogoutConfirmOpen(false)} title="Se déconnecter ?">
                <div className="flex flex-col gap-5">
                    <p className="text-sm text-muted">Voulez-vous vraiment vous déconnecter de votre compte StreamMusic ?</p>
                    <div className="flex justify-end gap-3 border-t border-white/5 pt-4">
                        <Button variant="ghost" size="md" onClick={() => setIsLogoutConfirmOpen(false)}>Annuler</Button>
