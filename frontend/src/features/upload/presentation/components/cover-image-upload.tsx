...
export function CoverImageUpload({ onImageSelect, onRemove, error, disabled }: CoverImageUploadProps) {
    ...
    const processFile = (selected: File) => {
        ...
        if (!ACCEPTED_TYPES.includes(selected.type)) {
            setUploadError('Format non supporté — utilise un JPG, PNG ou WEBP.')
            return
        }
        if (selected.size > MAX_IMAGE_SIZE_MB * 1024 * 1024) {
            setUploadError(`Image trop volumineuse (max ${MAX_IMAGE_SIZE_MB} Mo).`)
            return
        }
        ...
        <div className="flex flex-col gap-2">
            ...
            {!preview && !isUploading ? (
                ...
                >
                    ...
                    <span className="font-body text-sm text-ivory mt-2">
            {isDragging ? 'Relâchez pour déposer' : 'Ajouter une pochette'}
          </span>
                    <span className="text-xs text-muted mt-0.5">JPG, PNG, WEBP — 5 Mo max</span>
                </label>
            ) : isUploading ? (
                <div className="aspect-square w-full max-w-[200px] flex items-center justify-center border border-white/10 rounded-xl bg-surface-raised">
                    <Spinner size="md" />
                </div>
            ) : (
            ...
            <span className="text-[10px] bg-teal/90 text-ink px-2 py-0.5 rounded-full font-medium">
              ✓
            </span>
                    </div>
                </div>
