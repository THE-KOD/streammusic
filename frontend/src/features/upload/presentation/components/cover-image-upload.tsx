...
export function CoverImageUpload({ onImageSelect, onRemove, error, disabled }: CoverImageUploadProps) {
    ...
    return (
        <div className="flex flex-col gap-2">
            <input type="file" accept={ACCEPTED_TYPES.join(',')} onChange={handleFileChange} ref={inputRef} className="hidden" id="cover-upload" disabled={disabled || isUploading} />
            {!preview && !isUploading ? (
                <label htmlFor="cover-upload" className={clsx('flex flex-col items-center justify-center aspect-square w-full max-w-[200px] border-2 border-dashed rounded-lg cursor-pointer transition-colors', displayError ? 'border-danger bg-danger/5' : 'border-white/10 hover:border-teal hover:bg-surface/50', disabled && 'opacity-50 cursor-not-allowed')}>
                    <Image className="w-8 h-8 text-muted" />
                    <span className="font-body text-sm text-ivory mt-1">Ajouter une pochette</span>
                </label>
            ) : isUploading ? (
                <div className="aspect-square w-full max-w-[200px] flex items-center justify-center border border-white/10 rounded-lg bg-surface-raised"><Spinner size="md" /></div>
            ) : (
                <div className="relative aspect-square w-full max-w-[200px] rounded-lg overflow-hidden border border-white/10">
                    ...
                    <div className="absolute top-1 right-1">
                        <Button variant="ghost" size="sm" onClick={handleRemove} aria-label="Retirer la pochette" className="bg-black/50 text-white hover:bg-black/70 p-1"><X className="w-4 h-4" /></Button>
                    </div>
                </div>
            )}
