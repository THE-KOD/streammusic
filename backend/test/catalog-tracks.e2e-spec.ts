...
describe('Catalog Tracks (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        // Artiste principal
        // ============================================================
        const resArtist = await request(app.getHttpServer())
            .post('/auth/register')
            .send({
                pseudo: pseudoArtist,
                email: emailArtist,
        ...
        // L'utilisateur artiste doit également être administrateur
        // car /genres et /tracks/:id/moderer sont maintenant protégés
        // par AdminGuard.
        ...
        // ============================================================
        // Second artiste — pour tester le refus d'ajout de titre
        // dans l'album d'un autre artiste
        // ============================================================
        ...
        // Création du genre
        // ============================================================
        // tokenArtist appartient maintenant à un administrateur,
        // donc POST /genres est autorisé.
        ...
        // ============================================================
        // Création de l'album de l'artiste principal
        // ============================================================
        const resAlbum = await request(app.getHttpServer())
            .post('/albums')
            .set('Authorization', `Bearer ${tokenArtist}`)
            .send({
                titre: 'Album Test',
                dateSortie: '2026-01-01',
            });

        ownAlbumId = resAlbum.body.id;

        // ============================================================
        // Création de l'album du second artiste
        // ============================================================
        const resOtherAlbum = await request(app.getHttpServer())
            .post('/albums')
            .set('Authorization', `Bearer ${tokenOther}`)
            .send({
                titre: 'Album Autre',
    ...
    it('GET /tracks — le catalogue public ne montre pas un titre EN_ATTENTE', async () => {
        const res = await request(app.getHttpServer())
            .get(`/tracks?artisteId=${artistId}`)
            .expect(200);

        expect(
            res.body.find((t: any) => t.id === trackId),
        ).toBeUndefined();
    });
