...
describe('Suggestions (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        const resListened = await request(app.getHttpServer()).post('/tracks')
            .set('Authorization', `Bearer ${token}`)
            .send({ titre: 'Deja Ecoute', genreId: resGenre.body.id, duree: 200, fichierAudioUrl: 'https://x.com/a.mp3' });
        listenedTrackId = resListened.body.id;
        await request(app.getHttpServer()).patch(`/tracks/${listenedTrackId}/moderer`)
            .set('Authorization', `Bearer ${token}`).send({ statut: 'VALIDE' });

        const resCandidate = await request(app.getHttpServer()).post('/tracks')
            .set('Authorization', `Bearer ${token}`)
            .send({ titre: 'Candidat Suggestion', genreId: resGenre.body.id, duree: 180, fichierAudioUrl: 'https://x.com/b.mp3' });
        candidateTrackId = resCandidate.body.id;
        await request(app.getHttpServer()).patch(`/tracks/${candidateTrackId}/moderer`)
            .set('Authorization', `Bearer ${token}`).send({ statut: 'VALIDE' });

        await request(app.getHttpServer()).post('/listening-history')
            .set('Authorization', `Bearer ${token}`).send({ titreId: listenedTrackId, dureeEcoutee: 150 });
    });
    ...
    it('GET /suggestions/mine — vide avant toute génération', async () => {
        const res = await request(app.getHttpServer()).get('/suggestions/mine')
            .set('Authorization', `Bearer ${token}`).expect(200);
        expect(res.body).toEqual([]);
    });

    it('POST /suggestions/mine/generate — suggère le candidat du même genre, pas le titre déjà écouté', async () => {
        const res = await request(app.getHttpServer()).post('/suggestions/mine/generate')
    ...
    it('GET /suggestions/mine — reflète la dernière génération persistée', async () => {
        const res = await request(app.getHttpServer()).get('/suggestions/mine')
            .set('Authorization', `Bearer ${token}`).expect(200);
        expect(res.body.some((s: any) => s.titreId === candidateTrackId)).toBe(true);
    });
});