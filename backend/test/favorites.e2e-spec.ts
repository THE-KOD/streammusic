...
describe('Favorites (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        // Un artiste qui publie un titre + un album,
        // et un fan distinct qui les met en favori
        ...
        const artistId = resArtist.body.utilisateur.id;
        ...
        // L'artiste doit également être administrateur,
        // car POST /genres est maintenant protégé par AdminGuard.
        ...
        const resFan = await request(app.getHttpServer())
            .post('/auth/register')
            .send({
                pseudo: pseudoFan,
                email: emailFan,
    ...
    afterAll(async () => {
        ...
        // tant qu'un titre le référence encore.
        await dataSource.query(
            'DELETE FROM utilisateur WHERE email IN (?, ?)',
            [emailArtist, emailFan],
        );
    ...
    it('POST /favorites/tracks/:id — refuse un titre inexistant', async () => {
        await request(app.getHttpServer())
            .post('/favorites/tracks/00000000-0000-0000-0000-000000000000')
    ...
    it('GET /favorites/tracks — le titre apparaît dans la liste', async () => {
        ...
        expect(
            res.body.some((t: any) => t.id === trackId),
        ).toBe(true);
    });
    ...
    it('POST /favorites/albums/:id puis GET — sauvegarde un album', async () => {
        ...
        expect(
            res.body.some((a: any) => a.id === albumId),
        ).toBe(true);
    });
});