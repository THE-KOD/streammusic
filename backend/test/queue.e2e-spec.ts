...
describe('Queue (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        // POST /genres est désormais protégé par AdminGuard.
        // Le même utilisateur de test cumule donc les rôles artiste + admin.
        ...
        const resTrack1 = await request(app.getHttpServer())
            .post('/tracks')
            .set('Authorization', `Bearer ${token}`)
            .send({
                titre: 'Queue Titre 1',
        ...
        const resTrack2 = await request(app.getHttpServer())
            .post('/tracks')
            .set('Authorization', `Bearer ${token}`)
            .send({
                titre: 'Queue Titre 2',
    ...
    it("GET /queue — les titres sont dans l'ordre d'ajout", async () => {
        ...
        expect(res.body.map((t: any) => t.titreId)).toEqual([
            track1Id,
            track2Id,
        ]);
    });
