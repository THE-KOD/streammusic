...
async function attendreApparitionDansRecherche(
    ...
    for (let i = 0; i < tentatives; i++) {
        ...
        if (res.body.tracks.some((t: any) => t.id === trackId)) {
            return true;
        }
...
const trouve = await attendreApparitionDansRecherche(
    app,
    titreUnique,
    trackId,
);
