                ...
                <div className="max-h-60 overflow-y-auto space-y-1">
                    {filteredTracks.length === 0 ? (
                        ...
                        filteredTracks.map((track) => (
                            <Card
                                key={track.id}
                                className={`cursor-pointer transition-colors ${selectedId === track.id ? 'bg-surface-raised border-teal' : ''}`}
                                onClick={() => setSelectedId(track.id)}
                            >
                                <div className="flex items-center justify-between">
                                    <div>
                                        <p className="font-body text-ivory">{track.title}</p>
                                        <p className="text-sm text-muted">{track.artistName}</p>
                                    </div>
